...
void acceptorLoop()
{
	...
	for (;;)
	{
		...
		pair<pair<unsigned int, unsigned int>, pair <unsigned int, unsigned int>> keyPair = generate_keys();

		string keys_ = to_string(keyPair.first.first) + ' ' + to_string(keyPair.first.second);
		...
		string password = from_ascii(decrypt(to_ascii(password_), s.length(), keyPair.second), password.length());
